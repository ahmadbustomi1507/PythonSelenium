...
def combine_allure(folder_source,folder_destination):
    """
    Read all files,
    create server.js,
    then run server.js,
    """
    filename = os.path.basename(folder_source)
    cur_dir  = os.path.dirname(os.path.realpath(__file__))

    print("> folder_source to process is " + folder_source)
    print("> Checking for folder_source contents")

    files_should_be = ["index.html", "app.js", "styles.css"]

    for file in files_should_be:
        if not os.path.exists(folder_source + sep + file):
            raise Exception(f"ERROR: File {folder_source + sep + file} doesnt exists, but it should!")
    ...
    print("> Scanning folder_source for data files...")
    
    for path, dirs, files in os.walk(folder_source):
        if files:
            folder_source_url = re.sub(f"^{folder_source.rstrip(sep).replace(sep, re_sep)}{re_sep}", "", path)
            if folder_source_url and folder_source_url != folder_source:
                for file in files:
                    file_url = folder_source_url + sep + file
                    ext = file.split(".")[-1]
    ...
    with open(f"{folder_source}{sep}server.js", "w", encoding="utf8") as f:
        f.write(r"""
    ...
    size = os.path.getsize(f'{folder_source}{sep}server.js')
    print(f"server.js is build, it's size is: {size} bytes")

    print("> Copying file sinon-9.2.4.js into folder_source...")
    copyfile(cur_dir + f"{sep}sinon-9.2.4.js", folder_source + f"{sep}sinon-9.2.4.js")

    print("sinon-9.2.4.js is copied")

    print("> Reading index.html file")
    with open(folder_source + f"{sep}index.html", "r", encoding="utf8") as f:
        index_html = f.read()
    ...
    if "sinon-9.2.4.js" not in index_html:
        ...
        with open(folder_source + f"{sep}index.html", "w", encoding="utf8") as f:
    ...
    for tag in soup.findAll('script'):
        file_path = folder_source + sep + tag['src']
        print("...", tag, file_path)
    ...
    for tag in soup.findAll('link'):
        if tag['rel'] == ["stylesheet"]:
            file_path = folder_source + sep + tag['href']
            print("...", tag, file_path)
            with open(file_path, "r", encoding="utf8") as ff:
                file_content = ff.read()
    ...
    with open(folder_destination + f"{sep}{filename}.html", "w", encoding="utf8") as f:
        f.write(str(soup))
    f.close()
